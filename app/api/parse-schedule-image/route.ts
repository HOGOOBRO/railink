...
function buildPrompt(defaultYear: number, defaultMonth: number, imageCount: number, userName: string, airline: string): string {
  ...
  const lines: string[] = [
    ...
    '- startTime = the LEFTMOST value of line ③. endTime = the RIGHTMOST value. The sub-columns are narrow and easy to mix up — read the horizontal position of each value carefully before assigning it.',
