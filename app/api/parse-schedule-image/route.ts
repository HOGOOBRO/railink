...
function buildPrompt(defaultYear: number, defaultMonth: number, imageCount: number, userName: string, airline: string): string {
  ...
  const lines: string[] = [
    ...
    '',
    '### Layout (A) — calendar grid (코레일관광개발 "계획 개인별 승무근무표", m.korailtravel.com)',
    '- A 7-column week grid (일요일..토요일). Each day cell stacks three lines:',
    '  ① the day number ("3 일") and sometimes a small lone count digit next to it — that count is NOT a train number, ignore it;',
    '  ② duty code + 1-2 train numbers (e.g. "H1052 205 214");',
    '  ③ up to three time-like values in three narrow sub-columns.',
    '- CRITICAL — line ③ is [출근시각 clock-in | 근무시간 WORKED-HOURS | 퇴근시각 clock-out]. The MIDDLE value is a DURATION (the amount of hours worked that day, e.g. "17:13" = 17h 13m), NOT a time of day. NEVER output the middle value as startTime or endTime.',
    '- Assign STRICTLY by horizontal position: startTime = the LEFTMOST value of line ③, endTime = the RIGHTMOST value. Do NOT sort the three values and do NOT assume the smallest one is the clock-in — on short duties the middle DURATION is numerically SMALLER than the clock-in. Example cell: "09:47 | 07:33 | 18:20" = clock-in 09:47, worked 7h33m, clock-out 18:20 → startTime "09:47", endTime "18:20" (07:33 is discarded). Picking the middle value as startTime is the most common mistake — if the value you chose for startTime sits in the MIDDLE of the three, re-read the leftmost one.',
    '- A clock-out can be a small hour just past midnight (e.g. "00:48") — read its leading digit carefully (00:48, not 10:48) and apply the overnight rule below.',
    '- Self-check EVERY work day: (endTime − startTime), wrapping past midnight when clock-out is smaller, must be equal to or a few hours MORE than the middle duration (the gap is break time). If the endTime you picked equals the MIDDLE value, you picked the wrong sub-column — re-read the rightmost value from the image. Never invent or adjust a time to satisfy this check; always re-read.',
    '- Emit a row for EVERY day cell that shows anything, including cells holding only "S", "S(주휴)" or a lone "~". Do not skip any day. A lone "~" marks the previous day\'s overnight duty ending that morning: diaNr "~", no times, no train numbers.',
    '- trainNr: the number(s) shown next to the duty code (e.g. "205 214"). Join with " · ". Do NOT confuse them with the time values on line ③.',
    '- OVERNIGHT (박차/1박/야간): clock-out earlier than clock-in ⇒ the duty ends the next day. Write endTime in 24+ hour notation (01:08 → 25:08, 11:49 → 35:49).',
