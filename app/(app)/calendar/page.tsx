...
function monthShifts(entryOf: (iso: string) => ScheduleEntry | undefined, year: number, month: number, airline?: string, base?: string): MonthShift[] {
  ...
  for (let i = 0; i < flights.length - 1; i++) {
    ...
    const aAway = !!a.toAirport && !baseAirports.has(a.toAirport)
    const bAway = !!b.fromAirport && !baseAirports.has(b.fromAirport)
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (s.isDemo) {
        ...
        try {
          // 부팅 데이터 fetch 안전망. 무한 로딩의 근본(세션 락 무한 대기)은 lib/supabase.ts의
          // 시한부 락으로 해소됐으므로 이 타임아웃은 평소 거의 발동하지 않는다 — 네트워크가
          // 정말 느릴 때만 동작하는 보조 장치다. 6초 내 응답이 없으면 끊고 한 번 더 시도,
          // 끝내 실패하면 catch로 떨어져 캐시 prefill을 유지한 채 로딩 표시만 해제한다.
          const [remoteMine, remoteCols] = await retryWithTimeout(
            ...
            { tries: 2, timeoutMs: 6000, backoffMs: 400 },
          )
          if (!alive) return
  ...
  const monthPeople = useMemo<MonthPerson[]>(() => {
    ...
    const ppl: MonthPerson[] = [{
      ...
      shifts: monthShifts(iso => myByDate.get(iso), year, month, liveAirline, session.base),
    }]
    for (const c of compares) {
