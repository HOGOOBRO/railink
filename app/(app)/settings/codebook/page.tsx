...
export default function CodebookSettingsPage() {
  ...
  const t = useTranslations('settings.codebook')
  ...
  function handleSave(input: Omit<CodebookEntry, 'id'>) {
    ...
    if (sheet.type === 'create') {
      ...
      if (error === 'duplicate') { showToast(t('duplicate'), 'danger'); return }
      if (error === 'limit')      { showToast(t('limit'), 'danger'); return }
      setState(next)
    ...
    >
      <header className="h-topbar flex items-center justify-between gap-1 px-1.5 border-b border-line bg-surface shrink-0">
        <div className="flex items-center gap-1">
          ...
          <h3 className={`text-[18px] font-bold tracking-tight text-ink-900 ${fromUpload ? 'pl-2' : ''}`}>{t('title')}</h3>
        </div>
        <button
        ...
        {empty ? (
          <div className="mt-2 flex flex-col items-center gap-3 py-10 px-4 bg-surface border border-dashed border-line rounded-lg">
            <p className="text-callout text-ink-700 text-center leading-relaxed">
              {t.rich('emptyTitle', { br: () => <br /> })}
            </p>
            <Button variant="outline-brand" size="sm" onClick={startSeed}>
              {t('seedStart')}
            </Button>
            <p className="text-[11px] text-ink-300">{t('emptyHint')}</p>
          </div>
        ) : (
          <>
            <p className="px-1 pb-2 text-[11px] font-bold tracking-wider uppercase text-ink-500">
              {t.rich('registeredCount', { count: state.codes.length, n: (c) => <span className="font-en">{c}</span> })}
            </p>
      ...
      {fromUpload && (
        <div className="px-4 py-3 pb-[calc(12px+env(safe-area-inset-bottom))] border-t border-line bg-surface shrink-0">
          <p className="text-center text-[11px] text-ink-500 mb-2">{t('returnPrompt')}</p>
          <Button variant="primary" size="default" className="w-full" onClick={returnToUpload}>
            <ChevronLeftIcon size={16} /> {t('returnToInput')}
          </Button>
        </div>
      )}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={`text-caption text-ink-500 mt-1 ${code.isOff ? '' : 'font-en'}`}>
          {code.isOff ? t('off') : `${code.startTime} – ${code.endTime}`}
        </div>
      </div>
...
function CodeForm({
  ...
  function submit() {
    ...
    if (!labelTrim) { setError(t('errorNameRequired')); return }
    if (labelTrim.length > MAX_LABEL) { setError(t('errorNameTooLong', { max: MAX_LABEL })); return }
    if (!isOff && !timesOk) { setError(t('errorTimeFormat')); return }
    onSave({
      label: labelTrim,
      isOff,
    ...
    <div className="px-5 pt-1 pb-7">
      ...
      >
        <div className="text-left">
          <div className="text-[15px] font-semibold text-ink-900">{t('offCodeTitle')}</div>
          <div className="text-[11.5px] text-ink-500 mt-0.5">
            {t('offCodeDesc')}
          </div>
        </div>
        <span onClick={e => e.stopPropagation()}>
          <Switch on={isOff} onChange={v => { setIsOff(v); setError(null) }} ariaLabel={t('offCodeAriaLabel')} />
        </span>
      </div>
      ...
      {!isOff && (
        <>
          ...
          <p className="text-caption text-ink-500 mt-2">
            {t.rich('timeHint', {
              b: (c) => <strong className="text-ink-700 font-en">{c}</strong>,
              muted: (c) => <span className="text-ink-300">{c}</span>,
            })}
          </p>
        </>
      )}
