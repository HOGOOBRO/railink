...
/* 동시 getCurrentSession 호출을 하나로 합친다(single-flight). (app) 레이아웃의
 * AirlineTheme·OnboardingGate·각 페이지가 마운트 시 거의 동시에 호출하면, 각자
 * supabase.auth.getSession()(+profiles)을 띄워 부팅 임계경로가 직렬로 늘어졌다.
 * 진행 중 호출이 있으면 그 Promise를 그대로 돌려줘 한 번만 실행 → 부팅 시 세션
 * 해석이 빨라지고 중복 인증/profiles 왕복도 사라진다. */
let inflightSession: Promise<Session | null> | null = null
export function getCurrentSession(): Promise<Session | null> {
  if (inflightSession) return inflightSession
  inflightSession = resolveCurrentSession().finally(() => { inflightSession = null })
